...
test("overview and first install guide are usable", async ({ page }) => {
  ...
  await expect(controlStrip.getByText("源码源", { exact: true })).toBeVisible();
  await expect(controlStrip.getByText("PyPI", { exact: true })).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: "网络与工具" })).toBeVisible();
  const overviewNetworkTools = page.locator(".overview-settings-panel");
