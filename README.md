...
- Core JSONL 文件日志 + stdout/stderr 兜底日志
...
1. 环境预检：系统、Git、uv、默认端口、目录权限、磁盘、Core 源码、venv、源码源、PyPI 镜像。
...
5. 工具链与初始化运行时：未检测到 uv 时先下载到 `runtime/tools/uv`，再 clone/fetch/pull Core、安装 uv 托管 Python 3.12、执行 `uv sync --no-dev`。
...
所有失败会进入任务历史和日志页。任务历史支持取消当前运行任务，并对失败或已取消的初始化、启动、uv 安装、修复和 Core 更新任务重试。修复入口位于“环境与修复”，支持安装/修复隔离 uv、重跑 `uv sync`、清理 uv cache、重建 venv、重新 clone Core、Core 更新/回滚、运行时备份，以及打开/复制隔离目录。
...
- Core 配置编辑器可读取 `data/config.json`、`data/configs`、`data/plugins_configs`、`data/ai_core` 下的 JSON 配置；保存前校验类型，自动备份，敏感字段只遮蔽展示
- 启动命令：
...
敏感信息会自动遮蔽，包括 `WS_TOKEN`、`REGISTER_CODE` 和代理认证信息。GSDesk v1 不包含崩溃上报、匿名统计或诊断包自动上传，详见 [隐私与遥测](docs/PRIVACY.md)。
...
推送 `v*` tag 会触发 Release workflow，生成 Windows NSIS 和 macOS app/dmg 产物，计算平台独立的 `SHA256SUMS-*.txt`，并上传到 GitHub Draft Release。v1 不做静默安装包替换，壳更新只提示版本和下载入口。
...
Windows 必须包含 NSIS `.exe`，macOS 必须包含 `.dmg` 和 `.app.zip`；脚本会生成并复核 `SHA256SUMS-*.txt`。
